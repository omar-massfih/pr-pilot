...
class MemoryService:
    ...
    def _make_profiler(self) -> AgentProvider:
        ...
        if self.config.memory.profile_model:
            provider = ProviderConfig(provider.name, self.config.memory.profile_model)
    ...
    def add_project(self, path: Path) -> Project:
        ...
        if remote:
            remote_rows = self.db.connection.execute(
                "SELECT id,path FROM projects WHERE remote_url = ?", (remote,)
            ).fetchall()
        ...
        now = _now()
        self.db.connection.execute(
            "INSERT INTO projects(id,name,path,remote_url,created_at,updated_at) VALUES(?,?,?,?,?,?)",
            (project_id, name, str(top), remote, now, now),
        )
        self.db.connection.commit()
    ...
    def index_project(self, reference: str | Path, *, force: bool = False) -> dict:
        project = self.db.resolve_project(reference)
        if not project.path.is_dir():
            raise AgentShipError(f"Registered project path no longer exists: {project.path}")
        commit = str(_git(project.path, "rev-parse", "HEAD")).strip()
        paths = str(_git(project.path, "ls-tree", "-r", "--name-only", "-z", "HEAD")).split("\0")
        candidates = [path for path in paths if path and self._include_path(path)]
        seen: set[str] = set()
        new_chunks: list[tuple[int, str]] = []
        changed_files = 0
        for relative in candidates:
            raw = _git(project.path, "show", f"HEAD:{relative}", binary=True)
...
"""
        ...
        output = self.profiler.invoke(prompt, repo=project.path, write=False)
